...
export function CustomNode({ data, selected }: NodeProps<CustomNodeType>) {
  ...
  return (
    <div
      className={`bg-white rounded-md shadow-sm border ${borderColor} ${selectedClass} min-w-[160px] transition-all duration-200`}
    >
      <Handle
        type="source"
        position={Position.Top}
        id="top"
        className="w-2.5 h-2.5 bg-slate-400 border-white border-[1px]"
      />
      <Handle
        type="source"
      ...
      <div className="flex items-center justify-between px-3 py-1.5 border-b border-slate-100 bg-slate-50/50 rounded-t-md">
        <div className="flex items-center gap-1.5">
          <span className="text-sm" title="Python">
            🐍
          </span>
          <span className="text-[11px] font-semibold text-slate-700 font-mono tracking-wide">
            {data.filename}
          </span>
        </div>
        ...
        <div className="flex items-center ml-2">
          {data.status === 'success' && (
            <div className="w-2 h-2 rounded-full bg-green-500" title="Success" />
          )}
          {data.status === 'running' && (
          ...
          {(!data.status || data.status === 'idle') && (
            <div className="w-2 h-2 rounded-full bg-slate-300" title="Idle" />
          )}
        </div>
      </div>

      {/* Body */}
      <div className="px-3 py-2.5 text-xs text-slate-700">{data.label}</div>
